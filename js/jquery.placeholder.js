...
Automatically activates on dom-ready using:
...
You can also have the color set for browsers with native support
...
(function($){
		...
		this.each(function(){
			...
			label	.attr('id', input.attr('id') + "_placeholder")
					...
					.css({
						...
						'padding-top': parseInt(input.css('padding-top')) + parseInt(input.css('border-top-width') + 'px'),
						'padding-left': parseInt(input.css('padding-left')) + parseInt(input.css('border-left-width') + 'px'),
						'opacity': 0.4,
						'cursor': 'text'
					})
	...
	function bindEvents() {
		...
		$('.' + ph)
			.on('keydown',itemFocus)
			.on('mousedown',itemFocus)
			.on('mouseup',itemFocus)
			.on('mouseclick',itemFocus)
			.on('focus',itemFocus)
			.on('focusin',itemFocus)
			.on('blur',itemBlur)
			.on('focusout',itemBlur)
			.on('change',itemChange);
			;
		$('.' + phl)
			.on('click', function() {  $($(this).data(ph)).focus(); })
			.on('mouseup', function() {  $($(this).data(ph)).focus(); });
		bound = true;
